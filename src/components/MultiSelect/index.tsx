...
function MultiSelect<T>({
	...
	return (
		<StyledMultiSelect {...styleProps} $icons={icons}>
			<div onClick={() => displayMenuState.set((prev) => !prev)} ref={mainDivRef}>
				{leftIcon || ""}
				<p>
					{" "}
					{placeholder} {"("}
					{optionState.value.length}/{options?.length || 0}
					{")"}{" "}
				</p>
				{rightIcon || ""}
			</div>
			<ul ref={menuUlRef} className={displayMenuState.value ? "showing" : "hidden"}>

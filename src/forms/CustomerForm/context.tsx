...
type CustomerContextProps = {
	...
	operatorsState: CustomState<Array<User>>;
...
const CustomerProvider = ({ children }: CustomerProviderProps) => {
	...
	const operatorsState = useCustomState<Array<User>>([]);
	...
	const onClickAddContact = () => {
		customerDataState.set((prev) => {
			...
			if (
				updatedCustomerData.CONTATOS &&
				updatedCustomerData.CONTATOS.some((c) => c.CODIGO && c.CODIGO === selectedContactState.value.CODIGO)
			) {
				updatedCustomerData.CONTATOS = updatedCustomerData.CONTATOS.filter(
					(c) => c.CODIGO !== selectedContactState.value.CODIGO
				);
			}

			updatedCustomerData.CONTATOS = [
				...(updatedCustomerData.CONTATOS || []),
				{ FILHOS: 0, ...selectedContactState.value, CODIGO },
			];
